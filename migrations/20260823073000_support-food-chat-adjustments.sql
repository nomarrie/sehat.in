...
ADD COLUMN applied_recommendation_set_id UUID
  REFERENCES public.nutrition_recommendation_sets(id) ON DELETE SET NULL;

CREATE INDEX chat_messages_applied_recommendation_set_idx
  ON public.chat_messages (applied_recommendation_set_id);
...
BEGIN
  ...
  IF jsonb_typeof(v_message.adjustment_payload->'meals') <> 'array' THEN
    RAISE EXCEPTION 'food adjustment payload is invalid';
  END IF;
  v_meals := v_message.adjustment_payload->'meals';
  IF jsonb_array_length(v_meals) <> 4
    OR (SELECT COUNT(DISTINCT meal.value->>'mealType') FROM jsonb_array_elements(v_meals) AS meal(value)) <> 4
  THEN
    RAISE EXCEPTION 'food adjustment must contain four distinct meals';
  END IF;
  ...
  FOR v_meal_index IN 0..3 LOOP
    v_meal := v_meals->v_meal_index;
    IF jsonb_typeof(v_meal->'ingredients') <> 'array'
    ...
    THEN
      RAISE EXCEPTION 'food adjustment details are invalid';
    END IF;
