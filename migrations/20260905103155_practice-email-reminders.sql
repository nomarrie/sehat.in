CREATE TABLE public.practice_email_deliveries (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  package_id UUID NOT NULL REFERENCES public.exercise_packages(id) ON DELETE CASCADE,
  reminder_date DATE NOT NULL,
  reminder_kind TEXT NOT NULL CHECK (reminder_kind IN ('new_session', 'missed_practice')),
  ...
  CONSTRAINT practice_email_deliveries_user_date_key UNIQUE (user_id, reminder_date),
  CONSTRAINT practice_email_deliveries_sent_state_check CHECK (
    (status = 'sent' AND sent_at IS NOT NULL AND provider_message_id IS NOT NULL)
    OR (status <> 'sent' AND sent_at IS NULL)
  )
);

CREATE INDEX practice_email_deliveries_status_attempt_idx
  ON public.practice_email_deliveries (status, last_attempted_at)
  WHERE status <> 'sent';

CREATE INDEX practice_email_deliveries_user_created_idx
  ON public.practice_email_deliveries (user_id, created_at DESC);

CREATE TRIGGER practice_email_deliveries_updated_at
BEFORE UPDATE ON public.practice_email_deliveries
FOR EACH ROW EXECUTE FUNCTION system.update_updated_at();

ALTER TABLE public.practice_email_deliveries ENABLE ROW LEVEL SECURITY;

REVOKE ALL ON public.practice_email_deliveries FROM PUBLIC, anon, authenticated;
GRANT SELECT, INSERT, UPDATE ON public.practice_email_deliveries TO project_admin;
